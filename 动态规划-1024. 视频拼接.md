...
同时我们还需要记录上一个被使用的子区间的结束位置为 pre，每次我们越过一个被使用的子区间，就说明我们要启用一个新子区间，这个新子区间的结束位置即为当前的 last。也就是说，每次我们遇到 i==pre，即当前位置就是上一个字区间的结束位置，则说明我们用完了一个被使用的子区间，需要开启一个新的子空间。这种情况下我们让答案加 1，并更新 pre 即可。
